 <?php
    ...
    if (
        ...
        $habitats = [
            ['id' => 'H-SVN', 'name' => 'Savane Centrale'],
            ['id' => 'H-JGL', 'name' => 'Canopée Mystérieuse'],
            ['id' => 'H-MRS', 'name' => 'Marais Tropical'],
            ['id' => 'H-DSR', 'name' => 'Désert Aride'],
        ];
        ...
        $statuses = ['visible', 'soin', 'invisible'];
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if (isset($_POST['create_animal'])) {
                ...
                // Validation minimale
                if (empty($_POST['name']) || empty($_POST['species']) || empty($_POST['habitat_id']) || empty($_POST['guide_id'])) {
                    $error_message = "Veuillez remplir tous les champs obligatoires (Nom, Espèce, Habitat, Guide).";
                } else {
                    // insert_new_animal($_POST, $_FILES);

                    // Simuler l'obtention du nouvel ID
                    $new_animal_id = 'AN-101';

                    // Si réussi, rediriger ou afficher un message de succès
                    $success_message = "Le nouvel animal '{$_POST['name']}' ({$_POST['species']}) a été ajouté avec succès (ID: {$new_animal_id}) !";

                    // Optionnellement, effacer les données du formulaire ou rediriger
                    // header("Location: admin_animal_details.php?id={$new_animal_id}&status=created"); exit();
                }
            }
        }
    } else {
         ...
         <div class="h-full flex flex-col justify-between p-4">
             ...
             <div class="border-t border-gray-200 dark:border-gray-800 pt-4 px-2">
                 <div class="flex items-center gap-3">
                     <div
                         classclass="bg-gray-200 h-9 w-9 rounded-full flex items-center justify-center text-gray-500 font-bold text-xs overflow-hidden">
                 ...
                 <form method="POST" action="admin_add_animal.php" enctype="multipart/form-data" class="flex flex-col gap-8">
                     ...
                     <section class="p-6 bg-surface-light dark:bg-surface-dark rounded-xl shadow-xl border border-gray-100 dark:border-gray-800">
                         ...
                         <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                             <div>
                                 <label for="name" class="block text-sm font-medium text-text-secondary-light dark:text-text-secondary-dark mb-1">Nom de l'Animal <span class="text-red-500">*</span></label>
                                 <input type="text" id="name" name="name" required
                                     class="w-full px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-lg bg-background-light dark:bg-surface-dark/50 focus:border-primary focus:ring-primary text-text-light dark:text-text-dark" placeholder="Ex: Atlas" />
                             </div>

                             <div>
                                 <label for="species" class="block text-sm font-medium text-text-secondary-light dark:text-text-secondary-dark mb-1">Espèce <span class="text-red-500">*</span></label>
                                 <input type="text" id="species" name="species" required
                                     class="w-full px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-lg bg-background-light dark:bg-surface-dark/50 focus:border-primary focus:ring-primary text-text-light dark:text-text-dark" placeholder="Ex: Lion de l'Atlas" />
                             </div>
                         </div>

                         <div class="mt-4">
                             <label for="description" class="block text-sm font-medium text-text-secondary-light dark:text-text-secondary-dark mb-1">Description (Fiche publique)</label>
                             <textarea id="description" name="description" rows="4"
                                 class="w-full px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-lg bg-background-light dark:bg-surface-dark/50 focus:border-primary focus:ring-primary text-text-light dark:text-text-dark resize-none" placeholder="Description de l'animal, son historique, et son rôle dans le zoo."></textarea>
                         </div>
                     </section>
                     ...
                     <section class="p-6 bg-surface-light dark:bg-surface-dark rounded-xl shadow-xl border border-gray-100 dark:border-gray-800">
                         ...
                         <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                             <div>
                                 <label for="habitat_id" class="block text-sm font-medium text-text-secondary-light dark:text-text-secondary-dark mb-1">Habitat Affecté <span class="text-red-500">*</span></label>
                     ...
                     <section class="p-6 bg-surface-light dark:bg-surface-dark rounded-xl shadow-xl border border-gray-100 dark:border-gray-800">
                         ...
                         <div class="space-y-4">
                             <div>
                                 <label for="status" class="block text-sm font-medium text-text-secondary-light dark:text-text-secondary-dark mb-1">Statut Actuel</label>
                                 <select id="status" name="status" required
                                     class="w-full px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-lg bg-background-light dark:bg-surface-dark/50 focus:border-primary focus:ring-primary text-text-light dark:text-text-dark">
                                     <?php foreach ($statuses as $status) : ?>
                                         <option value="<?= htmlspecialchars($status) ?>" <?= $status === 'visible' ? 'selected' : '' ?>>
                                             <?= ucfirst(htmlspecialchars($status)) ?>
                                         </option>
                                     <?php endforeach; ?>
                                 </select>
                                 <p class="text-xs text-text-secondary-light mt-1">**Visible** : Apparaît sur les visites. **Soin** : Nécessite une attention, reste visible avec avertissement. **Invisible** : Masqué.</p>
                             </div>

                             <div>
                                 <label for="image_upload" class="block text-sm font-medium text-text-secondary-light dark:text-text-secondary-dark mb-1">Image de l'Animal <span class="text-red-500">*</span></label>
                                 ...
                                 <p class="text-xs text-text-secondary-light mt-1">L'image sera utilisée pour la fiche de l'animal et les aperçus.</p>
                             </div>
                         </div>
                     </section>

                     <div class="flex justify-end gap-4 pt-4 border-t border-gray-100 dark:border-gray-800">
                         <a href="admin_animaux.php" class="flex items-center gap-2 px-6 py-2.5 rounded-lg border border-gray-200 dark:border-gray-700 text-text-light dark:text-text-dark font-bold hover:bg-gray-100 dark:hover:bg-gray-800/50 transition-colors">
                             <span class="material-symbols-outlined text-[20px]">cancel</span>
                             <span>Annuler</span>
                         </a>
                         <button type="submit" name="create_animal" class="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-6 py-2.5 rounded-lg font-bold shadow-lg shadow-green-600/30 transition-all transform hover:scale-[1.01]">
                             <span class="material-symbols-outlined text-[20px]">save</span>
                             <span>Créer l'Animal</span>
                         </button>
                     </div>
                 </form>
